...
def main():
    ...
    frames = preprocess_frames(frames)[None].to(device)
    ...
    print('SIZE', args.step, grid_size)
    ...
    for t in tqdm(q_ts, desc="query frames"):
        ...
        if len(all_points_t) > 0:
            ...
            for points in tqdm(np.array_split(all_points_t, axis=0, indices_or_sections=num_chunks), leave=False, desc="points"):
                points = torch.from_numpy(points.astype(np.float32))[None].to(device)  # Add batch dimension
                with torch.inference_mode(), torch.cuda.amp.autocast():
                    preds = model(frames, points)
                tracks = preds["tracks"][0].detach().cpu().numpy()
                occlusions = preds["occlusion"][0].detach().cpu().numpy()
                expected_dist = preds["expected_dist"][0].detach().cpu().numpy()
                tracks = transforms.convert_grid_coordinates(tracks, (resize_width - 1, resize_height - 1), (width - 1, height - 1))
                outputs.append(np.concatenate([tracks, occlusions[..., None], expected_dist[..., None]], axis=-1))
